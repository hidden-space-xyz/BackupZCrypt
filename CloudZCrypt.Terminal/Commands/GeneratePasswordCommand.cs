...
internal sealed class GeneratePasswordCommand(IPasswordService passwordService)
{
    public void Execute()
    {
        AnsiConsole.Write(new Rule("[bold cyan]Password Generator[/]").RuleStyle(Style.Parse("grey")));
    ...
    private static PasswordGenerationOptions PromptOptions()
    {
        List<string> optionChoices = AnsiConsole.Prompt(
            new MultiSelectionPrompt<string>()
                ...
                .AddChoices("Uppercase (A-Z)", "Lowercase (a-z)", "Numbers (0-9)", "Special characters (!@#…)")
        ...
        int length)
    {
        ...
        Panel passwordPanel = new(
            new Rows(
                ...
                new Text(""),
                new Markup($"[dim]Strength:[/] [bold green]{analysis.Strength}[/] — {Markup.Escape(analysis.Description)}"),
        ...
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine("[yellow]? Store this password securely — it cannot be recovered if lost![/]");
    }
}
