    ...
    IReadOnlyList<ICompressionStrategy> compressionStrategies)
{
    public async Task ExecuteAsync(EncryptOperation operation)
    {
        ...
        bool useEncryption = await AnsiConsole.ConfirmAsync(
            $"[green]{(operation == EncryptOperation.Encrypt ? Messages.EncryptBackupPrompt : Messages.BackupEncryptedPrompt)}[/]");

        bool useCompression;
        ICompressionStrategy? selectedCompression = null;

        if (operation == EncryptOperation.Encrypt)
        {
            useCompression = await AnsiConsole.ConfirmAsync(
                $"[green]{Messages.CompressBackupPrompt}[/]");

            if (useCompression)
            {
                selectedCompression = PromptStrategy(
        ...
        {
            useCompression = !useEncryption && await AnsiConsole.ConfirmAsync(
                $"[green]{Messages.BackupCompressedPrompt}[/]");
        }

        if (useEncryption && operation == EncryptOperation.Encrypt)
        {
            await ExecuteEncryptAsync(
                operationName,
                sourcePath,
                destinationPath,
                selectedCompression);
        ...
        {
            await ExecuteDecryptAsync(operationName, sourcePath, destinationPath);
        }
        else if (useCompression || selectedCompression is not null)
        {
            await ExecuteCompressOnlyAsync(
                operationName,
                sourcePath,
                destinationPath,
                operation,
    ...
    private async Task ExecuteEncryptAsync(
        string operationName,
        string sourcePath,
        string destinationPath,
        ICompressionStrategy? preSelectedCompression)
    {
        (string password, string confirmPassword, bool wasAutoGenerated) = PromptPasswords();
        ...
        bool useObfuscation = await AnsiConsole.ConfirmAsync(
            $"[green]{Messages.ObfuscateNamesPrompt}[/]");

        NameObfuscationMode obfuscationMode = NameObfuscationMode.None;
        INameObfuscationStrategy? selectedObfuscation = null;

        if (useObfuscation)
        {
            selectedObfuscation = PromptStrategy(
                Messages.NameObfuscationModePrompt,
                nameObfuscationStrategies,
                s => $"{s.DisplayName} — {s.Summary}");
            obfuscationMode = selectedObfuscation.Id;
        }

        CompressionMode compressionMode = preSelectedCompression?.Id ?? CompressionMode.None;
    ...
    private async Task ExecuteDecryptAsync(
        ...
        string destinationPath)
    {
        string password = PromptDecryptionPassword();
    ...
    private async Task ExecuteCompressOnlyAsync(
        string operationName,
        string sourcePath,
        string destinationPath,
        ...
        ICompressionStrategy? preSelectedCompression)
    {
        // For compress: use pre-selected or prompt. For decompress: auto-detected from BZC header.
        CompressionMode compressionMode;

        if (operation == EncryptOperation.Encrypt)
        {
            if (preSelectedCompression is not null)
            {
                compressionMode = preSelectedCompression.Id;
            }
            else
            {
                ICompressionStrategy selectedCompression = PromptStrategy(
                    ...
                    compressionStrategies,
                    s => $"{s.DisplayName} — {s.Summary}");
                compressionMode = selectedCompression.Id;
            }
        }
        else
        {
            compressionMode = compressionStrategies[0].Id;
        }
        ...
        string ingName = operation == EncryptOperation.Encrypt
            ? Messages.Compressing
            : Messages.Decompressing;
        ...
        await RunOperationAsync(request, operationName, ingName);
    }

    private static async Task ExecutePlainCopyAsync(
        string operationName,
        string sourcePath,
        string destinationPath)
    {
                    ...
                    .StartAsync(async ctx =>
                    {
                        ...
                        foreach (string file in files)
                        {
                            ...
                            try
                            {
                                ...
                                using FileStream destStream = new(
    ...
    private (string Password, string ConfirmPassword, bool WasAutoGenerated) PromptPasswords()
    {
        ...
        if (string.IsNullOrWhiteSpace(password))
        {
            ...
            password = passwordService.GeneratePassword(autoGeneratedLength, autoGeneratedOptions);
