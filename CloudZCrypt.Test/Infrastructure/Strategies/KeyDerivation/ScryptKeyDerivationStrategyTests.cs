...
{
    private ScryptKeyDerivationStrategy _strategy = null!;

    [SetUp]
    public void SetUp()
    {
        _strategy = new ScryptKeyDerivationStrategy();
    }

    [Test]
    public void Id_ReturnsScrypt()
    {
        Assert.That(_strategy.Id, Is.EqualTo(KeyDerivationAlgorithm.Scrypt));
    }

    [Test]
    public void DisplayName_ContainsScrypt()
    {
        Assert.That(_strategy.DisplayName, Does.Contain("Scrypt"));
    }
